@elasticsearch
  ...
  Scenario: Games Resource should respond with filtered games when a valid genre UUID is given.
    ...
    And the JSON node "hits.hits[0]._source.uuid" should be equal to "08952aa6-e079-496a-8efa-cbb8465d9315"
    And the JSON node "hits.hits[0]._source.id" should be equal to "12"
  ...
  Scenario: Games Resource should respond with filtered games when multiple valid genres UUID are given.
    ...
    And the JSON node "hits.hits[0]._source.uuid" should be equal to "08952aa6-e079-496a-8efa-cbb8465d9315"
    And the JSON node "hits.hits[0]._source.id" should be equal to "12"
    And the JSON node "hits.hits[1]._source.uuid" should be equal to "f990d6af-d50d-4b35-a79a-72a1e12a7422"
    And the JSON node "hits.hits[1]._source.id" should be equal to "17"
