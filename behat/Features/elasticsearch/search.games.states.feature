...
Feature: Retrieve Games items from Elasticsearch
  ...
  Scenario: Games Resource should respond with filtered games when a valid store name is given.
    Given I send a "GET" request to "http://api.gos.test/search/games?page=0&states[]=pre_release"
  ...
  Scenario: Games Resource should respond with filtered games when multiple valid states names are given.
    Given I send a "GET" request to "http://api.gos.test/search/games?page=0&states[]=released&states[]=pre_release"
    Then the response status code should be 200
    And the response should be in JSON
  ...
  Scenario: Games Resource should respond with an error when a non-valid store name is given.
    Given I send a "GET" request to "http://api.gos.test/search/games?page=0&states[]=foo"
    Then the response status code should be 400
    And the response should be in JSON
    And the JSON node "errors.states" should exist
    And the JSON node "errors.states" should have 1 element
    And the JSON node "errors.states[0]" should be equal to "One or more of the given values is invalid."
